...
```{r}
ggplot(penguins, aes(x = flipper_length_mm)) + #information about the dataset
  geom_histogram(binwidth=3, color = "grey", fill="lightblue") + #make the figure
...
ggplot(penguins, aes(x = bill_length_mm, y = bill_depth_mm, color = species)) +
  geom_point() +
  theme_minimal() +
  scale_colour_manual(values = c("lightblue", "coral", "orchid")) + #change the color
...
ggplot(penguins, aes(x = body_mass_g, fill = species)) +
  ...
  theme_minimal() +
  scale_fill_manual(values = c("lightblue", "coral", "orchid"))
```
...
ggplot(penguins, aes(x = bill_depth_mm, y = flipper_length_mm, color = species)) +
  geom_point() +
  theme_minimal() +
  scale_colour_manual(values = c("lightblue", "coral", "orchid")) +
geom_point(size = 1.5) #change the size 
```
...
When I first input body_mass_g as the color variable, it gave me an error because I was trying to use a continuous variable to color points when I had only given in 3 colors. When I removed the manual colors it automatically colored the points on a scale.
